...
<template>
  <div
    ...
    class="q-message-avatar row flex-center"
    :class="sent ? 'q-message-avatar--sent' : 'q-message-avatar--received'"
    :style="{ backgroundColor: validatedUser.avatarColor, color: 'white' }"
  >
    <span
      v-if="validatedUser.name === currentUser.name"
      class="custom-avatar"
      >{{ validatedUser.avatar }}</span
    >
    <img
      v-if="validatedUser.name === systemUser.name"
